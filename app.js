...
class ModuleManager {
    ...
    getEvaluacion() {
        ...
        let sesionesHTML = '';
        data.evaluacion.plantillaSesiones.forEach((s, index) => {
            sesionesHTML += `
                <div class="evidencia-item">
                    <div class="evidencia-header">
                        <h4>${s.evaluacion === 'inicial' ? 'Evaluación Inicial' : `${s.evaluacion}ª Evaluación`} - ${s.fecha}</h4>
                        <button class="btn-delete" onclick="moduleManager.deleteSesionEvaluacion(${index})">🗑️ Eliminar</button>
                    </div>
                    <p>${s.notas}</p>
                </div>
            `;
        ...
        return `
            <div class="module-container">
                ...
                <div class="content-section">
                    <h3>📝 Plantilla para Sesiones de Evaluación</h3>
                    <p style="color: #7f8c8d; margin-bottom: 1.5rem;">Registra las sesiones de evaluación realizadas:</p>
                    
                    <div class="form-group">
                        <label>Evaluación:</label>
                        <select id="sesionEval" class="form-input">
                            <option value="inicial">Inicial</option>
                            <option value="1">Primera Evaluación</option>
                            <option value="2">Segunda Evaluación</option>
                            <option value="3">Tercera Evaluación/Final</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label>Fecha:</label>
                        <input type="date" id="sesionFecha" class="form-input" value="${new Date().toISOString().split('T')[0]}">
                    </div>

                    <div class="form-group">
                        <label>Acuerdos / Observaciones:</label>
                        <textarea id="sesionNotas" class="form-textarea" rows="4" placeholder="Acuerdos tomados, medidas propuestas, etc."></textarea>
                    </div>

                    <button class="btn-primary" id="btnAddSesion">➕ Añadir Sesión</button>

                    <div style="margin-top: 2rem;">
                        <h4 style="margin-bottom: 1rem;">Sesiones Registradas</h4>
                        ${sesionesHTML || '<p class="empty-state">No hay sesiones registradas aún</p>'}
                    </div>
                </div>
    ...
    initEvaluacionEvents() {
        ...
        document.getElementById('btnAddSesion').addEventListener('click', () => this.addSesionEvaluacion());
    }
    ...
    addSesionEvaluacion() {
        const evaluacion = document.getElementById('sesionEval').value;
        ...
        const data = storage.getData();
        data.evaluacion.plantillaSesiones.push({ evaluacion, fecha, notas });
        storage.saveData(data);
    ...
    deleteSesionEvaluacion(index) {
        if (confirm('¿Eliminar este registro de sesión?')) {
            const data = storage.getData();
            data.evaluacion.plantillaSesiones.splice(index, 1);
            storage.saveData(data);
            nav.switchModule('evaluacion');
        }
    }
            ...
            <div class="module-container">
                ...
                <div class="content-section">
                    ...
                    <div class="admin-actions">
                        <button class="btn-admin">📊 Generar Informe General</button>
                        <button class="btn-admin">📁 Exportar Datos</button>
                        <button class="btn-admin">⚙️ Configuración</button>
                    </div>
                </div>
            </div>
        `;
